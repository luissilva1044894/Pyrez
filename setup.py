...
def __getRequirements(filename='base'):
  ...
  for requirement in __readFile('requirements/{}'.format(filename if filename.endswith('.txt') else '{}.txt'.format(filename))).splitlines():
    if requirement:
      if requirement[:3].lower() == '-r ':
        ...
        pass
      else: requirements.append(requirement)
  return requirements
...
def __getMetadata(package_name='pyrez'):
  meta_ = {}
  exec(__readFile(f'{package_name}/__version__.py'), meta_)
  return meta_
_exec = __getMetadata()
...
if sys.version_info[:2] < (3, 6):
  print(f'ERROR: {NAME.capitalize()} requires at least Python 3.6 to run.')
  sys.exit(1)
...
class DocsCommand(BaseCommand):
  ...
  def run(self):
    pass#print(self.confirm('TESTING?!'))
class UploadCommand(BaseCommand):
  ...
  def run(self):
    ...
    call_(f'{sys.executable} setup.py sdist bdist_wheel --universal') #call([sys.executable, 'setup.py sdist bdist_wheel --universal'], shell=False)
    self.status(f'Uploading the {NAME.capitalize()} package to PyPI via Twine…')
    call_('twine upload dist/*')
    if self.confirm('Push tags'):
      self.status('Pushing git tags…')
      call_(f'git tag {VERSION}')#git tag v{0}
      call_('git push --tags')
    if self.confirm('Clear?'): #rm -r dist build *.egg-info
      self.recursive_delete('dist')
      self.recursive_delete('build')
      self.recursive_delete(f'{NAME}.egg-info')
    sys.exit()
...
setup(
  ...
  entry_points = {
    'console_scripts': [
      f'{NAME}={NAME}.command_line:main',#f'{NAME}-cli={NAME}.command_line:main',
    ],
  },
  ...
  #download_url=f'https://pypi.org/project/{NAME}/#files',
  #__getGithub(f'tarball/{VERSION}')
  download_url=f'{URL}/archive/{VERSION}.tar.gz',#__getGithub(f'archive/{VERSION}.tar.gz')
  ...
  python_requires='>=3.6.1', #python_requires='>=3.0, !=3.1.*, !=3.2.*, !=3.3.*, !=3.4.*, !=3.5.*, !=3.6.*, !=3.7.*, !=3.8.*, <4',
  setup_requires=__getRequirements('base'),
  ...
  project_urls={
    'Documentation': f'https://{NAME}.readthedocs.io/en/stable/',
